...
const setFinalRoundParticipants = (finalround, rounds, participants) => {
  ...
  const championshipwinner = championfinal.participants.find(
    (participant) => participant && participant.isWinner,
  );
  const participant = {
    ...(championshipwinner &&
      participants.find(
        (participant) => participant.id === championshipwinner.id,
      )),
  };
