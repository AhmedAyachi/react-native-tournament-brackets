...
export default function RoundView(props) {
  ...
  return (
    <View style={[css.roundview, props.style]}>
      <View style={css.row0}>
        <Text style={css.title} numberOfLines={1} ellipsizeMode="clip">
          {round.title}
        </Text>
      </View>
      <View style={css.row1}>
        {matches &&
          matches.map((match, i) => (
            <View style={css.section} key={`section${i}`}>
              {props.connected ? (
                <ConnectorView {...props.connectorStyle} />
              ) : (
                <></>
              )}
              <View
                style={css.matchcontainer}
                ref={refs.matchcontainers[i]}
...
const useMatchOffset = (refs, callback) => {
  new Promise((resolve) => {
    ...
    if (length > 1) {
      average =
        css.matchcontainer.marginVertical / 2 +
